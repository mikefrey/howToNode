  ...
  http.get(urls[1], function(res) {
    ...
    http.get(urls[2], function(res) {
      ...
      http.get(urls[3], function(res) {
        ...
        console.error('Error getting', urls[3], err)
      })
    }).on('error', function(err) {
      console.error('Error getting', urls[2], err)
    })
  }).on('error', function(err) {
    console.error('Error getting', urls[1], err)
  })
}).on('error', function(err) {
  console.error('Error getting', urls[0], err)
})